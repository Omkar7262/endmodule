...
class Client(BaseModel):
    client_code = models.CharField(max_length=100, verbose_name="Client Code", null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.client_code:
            ...
            if last_client and last_client.client_code:
                last_code = int(last_client.client_code.split('-')[-1])
                self.client_code = f"CL-{last_code + 1:04d}"
            else:
                self.client_code = "CL-0001"
